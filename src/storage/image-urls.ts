 ...
 *
 * Re-encodes to WebP quality 0.9 once, preserving generation metadata. Preview
 * and storage share those bytes rather than shipping the original PNG first.
 * Bytes go to character assets when the host can save them. If the
...
export async function publishImage(
  ...
  if (options.onPreview) {
    // Register the URL task before storage can yield to a viewer request. All
    // consumers join one encode of the final bytes, without reading the asset.
    const encodedBytes = bytes;
    const urlTask = Promise.resolve().then(async () => {
      encodeCount += 1;
      const url = await bytesToDataUrlAsync(encodedBytes, mime);
      setBlobUrl(id, url, encodedBytes.byteLength);
      notifyImageUrl(id, url);
      return url;
    }).finally(() => { if (encoding.get(id) === urlTask) encoding.delete(id); });
    encoding.set(id, urlTask);
    const preview = urlTask.then(async url => {
      try { await options.onPreview!(url); }
      catch (err) { dbg('image.preview.fail', { id, message: String((err as Error)?.message || err) }, 'warn'); }
      return url;
    });
    // A slow SafeDOM paint must not delay file persistence, nor should owner
    // lookup/asset registration delay showing an already encoded image.
    const [, url] = await Promise.all([
      idbPut('images', { id, png: bytes, mime, location: location || {}, asset_only: true }),
      preview,
    ]);
    span.end({ message: id, bytes: bytes.byteLength, src_bytes: png.byteLength, mime, has_url: Boolean(url), focus: true });
    return url;
  }
